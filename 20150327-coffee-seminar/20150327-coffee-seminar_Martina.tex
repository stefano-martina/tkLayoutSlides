...
\begin{frame}[fragile]
  \frametitle{Tracker design: material budget}
  \begin{center}
    \begin{tikzpicture}[edge from parent/.style={draw,-latex}, level 1/.style={sibling distance=3cm}, level distance=1cm]
      %arrows
      \node[powmat] (powmatn) at (0,0) {Power $\rightarrow$ Material};
      \onslide<2-> { \node[arrowR] (mormat) at ($ (powmatn.east) + (0.5,0) $) {MORE}; }
      \onslide<5-> { \node[arrowL] (lessmat) at ($ (powmatn.west) - (0.5,0) $) {LESS}; }

      \pause
      %under more
      \node[underboxr, below of=mormat] (higran) {Pileup \textasciitilde 200}
      child { node[underunderbox] {Higher granularity}}
...
%      child { node {+ consumption}}
      ...
      child { node[underunderbox, text width=2cm, font=\tiny] {Trigger
          rate: complete readout @100$\rightarrow$750~kHz}}
      child { node[underunderbox, text width=3cm, font=\tiny] {Concur to trigger
          formation: partial readout @ 40~MHz}};
...
\begin{frame}
  \frametitle{Modules}
  \begin{center}
    \includegraphics[width=\textwidth]{img/modules2.pdf}
  \end{center}
\end{frame}
...
\pause
\begin{itemize}
\item Tracker Barrel 2S
\pause
\item Tracker Barrel PS
\pause
\item Tracker Endcap Double Disk
\pause
\item Pixel barrel and endcap $\eta\rightarrow 4$
\end{itemize}
\end{frame}

\begin{frame}
  \frametitle{TEDD mechanics (2S \& PS)}
  \begin{center}
...
\begin{frame}{New feature}
  ...
  \end{block}
  \begin{center}
    \only<1>{\includegraphics[width=\textwidth]{img/electro-opto.png}}
    \only<2>{\includegraphics[width=\textwidth]{img/electro-opto1.pdf}}
    \only<3>{\includegraphics[width=\textwidth]{img/electro-opto2.pdf}}
    \only<4>{\includegraphics[width=\textwidth]{img/electro-opto3.pdf}}
  \end{center}
\end{frame}
...
\foreach \n in {1,...,6}{
  \begin{frame}
    \begin{center}
      \includegraphics[width=\textwidth]{img/backup\n.pdf}
    \end{center}
  \end{frame}
}
